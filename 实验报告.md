...
2. 指出输出中的异常捕获信息
3. 对照代码解释处理流程
4. 用 `objdump` 验证地址和指令长度
5. 强调 `advance_pc()` 的作用

**回答可能的追问**：
- **Q**: 为什么ebreak是2字节？
  - **A**: 因为RISC-V编译器启用了压缩指令扩展(-march=rv64imac)，ebreak可以被编码为16位的c.ebreak。

- **Q**: 如何确保异常处理不会死循环？
  - **A**: 通过修改tf->epc，使其指向下一条指令，sret返回后从新地址继续执行。

- **Q**: OpenSBI的警告是否影响功能？
  - **A**: 不影响。这只是OpenSBI在模拟CSR访问时的调试信息，实际的非法指令异常已被正确捕获和处理。
